...
for N in [1, 10, 100]:
    for ph in phs:
        ...
        for n in range(1, N+1, 1):            
            ...
            cmd = "gnina -r {recf} -l {ligf} --center_x {cenx} --center_y {ceny} --center_z {cenz} --size_x 20 --size_y 20 --size_z 20 -o {docf} --exhaustiveness 16 --verbosity 4 --cpu 20 --seed 123".format(recf = recFile, ligf = ligands, docf = dockFile, cenx = com[0], ceny = com[1], cenz = com[2])
            print(cmd)
            subprocess.run(cmd, shell=True, check=True)      

...
"""
drugdir = join(dcd_prefix, "possessActVal")
drugfiles = glob.glob(join(drugdir, '*.pdbqt'))
v = Vina(sf_name='vina', cpu=20, seed=123)
for N in [1, 10, 100]:
    for ph in phs:
        jobprefix = os.path.join(dcd_prefix, str(ph))
        ...
        for n in range(1, N+1, 1):            
            ...
            v.set_receptor(recFile)
            v.compute_vina_maps(center=com, box_size=[20, 20, 20])
            for drugIndex in range(0,len(drugfiles)):
                v.set_ligand_from_file(drugfiles[drugIndex])
                loc = "view/pH/%s/N/%s/n/%s/d/%s/job" % (ph, N, n, drugIndex)
                drugOutPrefix = join(dcd_prefix,loc)
                dockFile = join(drugOutPrefix,"receptor_vina_out.pdbqt")
                print(dockFile)
                # Dock the ligand
                v.dock(exhaustiveness=16, n_poses=10)
                v.write_poses(dockFile, n_poses=5, overwrite=True)
